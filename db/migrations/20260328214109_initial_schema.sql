...
CREATE TABLE recipe_ingredients (
    ...
    display_text  TEXT,   -- the name gotten from the source recipe
    order_index   INT
);
...
CREATE TABLE pantry_items (
    id               UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id          TEXT REFERENCES user_profiles(id) ON DELETE CASCADE,
    name             TEXT,   -- pantry items doesn't need to be exact name as USDA ingredient since it can come from OCR or more simple name
...
CREATE TABLE grocery_items (
    ...
    name             TEXT, -- name of the grocery item, not neccesarily ingredients name (ingredient names are too long)
    ingredient_id    UUID NOT NULL REFERENCES ingredients(id),
    quantity         DECIMAL(10,2),
    unit             TEXT,
...
CREATE OR REPLACE VIEW pantry_items_with_freshness AS
SELECT
    *,
    CASE
